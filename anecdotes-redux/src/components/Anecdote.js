...
const Anecdote = ({ anecdote }) => {
  ...
  const toVote = (id, content) => {
    dispatch(voteFor(id))
    dispatch(
      setNotification(`You voted "${content.slice(0, 40)}..."`, "info", 4000)
    )
  }
  const deleteById = id => {
    ...
    <Card variant="outlined">
      <CardContent>
        ...
        <Button
          ...
          color="secondary"
          size="small"
          onClick={() => toVote(anecdote.id, anecdote.content)}
          style={{ marginTop: "1.7em", marginRight: "1em" }}
        >
          vote
        </Button>
        <Button
          ...
          size="small"
          onClick={() => deleteById(anecdote.id)}
